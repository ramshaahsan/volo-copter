...
@SpringBootTest
class DownwardMovementExecutorTests {
    ...
    @Test
    fun testMoveDownward_MultipleCommands_ShouldPass() {
        ...
        Assertions.assertEquals(1, droneMovementInputService.drone.y)
        Assertions.assertEquals(5, droneMovementInputService.drone.z)
    }

    @Test
    fun testMoveDownward_SingleCommand_ShouldPass() {
        ...
        val command = Command(1, "DOWN", 5)
        droneMovementProcessor.executeCommand(currentPosition, command)

        Assertions.assertEquals(5, droneMovementInputService.drone.x)
        Assertions.assertEquals(0, droneMovementInputService.drone.y)
        Assertions.assertEquals(5, droneMovementInputService.drone.z)
    }
    @Test
    fun testMoveDownward_Collision() {
        ...
        Assertions.assertTrue(consoleOutput.contains("(0, -5, 0) -> (5, 0, 5)"))
        ...
        Assertions.assertTrue(consoleOutput.contains("CRASH"))
        Assertions.assertTrue(consoleOutput.contains("(0, 0, 0) -> (5, 0, 5)"))
    }
}